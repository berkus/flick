...
/*
 * This function is almost identical to `pres_c_interpose_internal_array'.  If
 * you change something here, you'll almost certainly need to change it there
 * as well.
 */
			      ...
			      const char *arglist_name)
{
	...
	new_mapping = pres_c_new_mapping(PRES_C_MAPPING_POINTER);
	new_mapping->pres_c_mapping_u_u.pointer.target = old_mapping;
	new_mapping->pres_c_mapping_u_u.pointer.arglist_name
