...
void khazana_mu_state::mu_server_func_reply(pres_c_server_func *sfunc,
					    pres_c_skel *sskel)
{
	 ...
	 * Khazana reply messages.  Similarly, strip away the ``collapsed
	 * union'' goo that represents the operation's reply code (which is a
	 * fixed value --- NOT an indicator of success or failure).  The client
	 * knows what operation it invoked.
	 */
	...
	add_stmt(cast_new_stmt_expr(
		cast_new_expr_call(
			cast_new_expr_name("flick_khazana_server_start_encode"),
			0)));
	...
	add_stmt(cast_new_stmt_expr(
		cast_new_expr_call(
			cast_new_expr_name("flick_khazana_server_end_encode"),
			0)));
}
...
void w_skel(pres_c_1 *pres, int stub_idx)
{
	...
	khazana_mu_state must(the_state,
			      MUST_DECODE | MUST_ALLOCATE | MUST_REQUEST,
			      assumptions, "server", KHAZANA_NO_SWAP);
	khazana_mu_state swap_must(the_state,
				   MUST_DECODE | MUST_ALLOCATE | MUST_REQUEST,
				   assumptions, "server", KHAZANA_SWAP);
	...
	for (unsigned int i = 0; i < skel->funcs.funcs_len; i++) {
		 ...
		 * Khazana because it is manifest in the object references.
		 *
		 ...
		 * reply by an Khazana version of `mu_server_func_reply()'.
		 */
		pres_c_inline inl;
	...
	w_printf("(FLICK_TARGET _this_obj, "
		 "flick_msg_t _in_msg, flick_msg_t *_out_msg)\n");
	w_printf("{\n");
