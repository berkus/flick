...
#define KHAZANA_NO_SWAP 0
#define KHAZANA_SWAP 1

class khazana_be_state : public be_state
{
...
struct khazana_mu_state : public mem_mu_state
{
	...
	khazana_mu_state(be_state *_state,
			 mu_state_op _op, 
			 int _assumptions, 
			 const char *which, 
			 int swap = KHAZANA_NO_SWAP);
	
	khazana_mu_state(const khazana_mu_state &must);
	mu_state *clone();
...
struct khazana_target_mu_state : public target_mu_state
{
	// This is whether or not the code should byteswap
	int should_swap;
	
	khazana_target_mu_state(be_state *_state, mu_state_op _op,
				int _assumptions, const char *which,
				int swap = KHAZANA_NO_SWAP);
	khazana_target_mu_state(const khazana_target_mu_state &must);
	virtual mu_state *clone(); 
	virtual const char *get_be_name();
	virtual const char *get_encode_name();
};

struct khazana_client_mu_state : public client_mu_state
{
	// This is whether or not the code should byteswap
	int should_swap;
	
	khazana_client_mu_state(be_state *_state, mu_state_op _op,
				int _assumptions, const char *which,
				int swap = KHAZANA_NO_SWAP);
	khazana_client_mu_state(const khazana_client_mu_state &must);
	virtual mu_state *clone(); 
	virtual const char *get_be_name();
	virtual const char *get_encode_name();
};
