...
void pg_corbaxx::p_client_stub_special_params(aoi_operation *ao,
					      stub_special_params *specials)
{
	...
	/*
	 * Set the object reference type and index.  Since this is C++
	 * we make the first parameter implicit since its the object,
	 * but it still needs to be in here for the backend to operate on.
	 */
	...
	p_indirect_type(cur_aoi_idx, &ptc);
	this_param->ctype = cast_new_pointer_type(ptc->
	...
	this_param->ctype =
		cast_new_reference_type(
			cast_new_type_name(
				 ...
				 * `pg_corbaxx::p_get_env_struct_type' has access
 ...
 */
void pg_corbaxx::p_client_stub_return_type(aoi_operation *ao, int mr,
					   cast_type *out_ctype,
