...
void pg_corbaxx::preprocess()
{
	...
	for (aoi_index = 0; aoi_index < in_aoi->defs.defs_len; ++aoi_index) {
		...
		if (in_aoi->defs.
		    defs_val[aoi_index].binding->kind != AOI_INTERFACE)
			continue;
		...
		for (attribs_index = 0;
			...
			this_op->flags = AOI_OP_FLAG_SETTER;
