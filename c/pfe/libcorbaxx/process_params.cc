 ...
 * interface operation.
 */
void pg_corbaxx::process_client_params(
	...
	pres_c_inline target_inl, pres_c_inline client_inl)
{
	...
	if (gen_server
		...
		pres_c_interpose_argument(&map, "params", "environment");
		...
		request_l4_inl->pres_c_inline_u_u.func_params_i.slots.
			...
			= pres_c_new_inline_atom(env_ref_index, map);
	}
	
	/*
	 * The PRES_C for handling the environment in the reply is constructed
	 * by `pg_corbaxx::p_do_exceptional_case' and its associates.
	 *
 ...
 */
void pg_corbaxx::process_server_params(
	cast_func_type *cfunc,
	stub_special_params *specials,
	mint_ref request_ref, mint_ref reply_ref,
	aoi_operation *ao,
...
{
	/*
	 * Call the `pg_corbaxx' version of this method!  This is important!
	 * Who knows what a derived class might do differently between client
	 * and server!
	 */
	pg_corbaxx::process_client_params(cfunc,
					  specials,
					  request_ref, reply_ref,
					  ao,
					  request_l4_inl, reply_l4_inl,
