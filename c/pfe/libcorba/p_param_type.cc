			    ...
			    cast_type *out_ctype, pres_c_mapping *out_mapping)
{
	...
	corba_alloc_always.pres_c_allocation_u_u.val.alloc_init = 0;
	...
	corba_ad_always.pres_c_allocation_u_u.val.flags
		...
		PRES_C_DEALLOC_ON_FAIL;
	corba_ad_always.pres_c_allocation_u_u.val.alloc_init = 0;
	...
	*out_mapping = ptn->get_mapping();
	...
	} else if (is_variable && !is_object && !is_string) {
		/*
	...
	case AOI_DIR_IN:
		...
		if ((t->kind == AOI_STRUCT)
			...
			pres_c_interpose_indirection_pointer(
			    out_ctype, out_mapping, in_pointer_alloc);
		break;
	...
	case AOI_DIR_INOUT:
		...
		if (is_array)
			...
			pres_c_interpose_indirection_pointer(
			    out_ctype, out_mapping, inout_pointer_alloc);
		break;
	...
	case AOI_DIR_OUT:
		...
		if (is_array && !is_variable)
			...
			pres_c_interpose_indirection_pointer(
			    out_ctype, out_mapping, out_pointer_alloc);
		break;
