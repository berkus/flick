...
pres_c_inline pg_state::p_inline_struct(aoi_struct *as,
					p_type_collection *inl_ptc,
					cast_type inl_ctype)
{
	pres_c_inline		inl   = pres_c_new_inline_struct(as->slots.
								 slots_len);
	pres_c_inline_struct	*sinl = &(inl->pres_c_inline_u_u.struct_i);
	
	aoi_struct_slot		*slot;
	...
	for (unsigned int i = 0; i < as->slots.slots_len; i++) {
		slot = &(as->slots.slots_val[i]);
