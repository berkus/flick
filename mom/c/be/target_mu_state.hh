...
#ifndef _mom_c_pbe_target_h_
#define _mom_c_pbe_target_h_

#include <mom/types.h>
#include <mom/c/be/mu_state.hh>

/*
 * Target reference marshaler.
 *
 * This is a very simple mu_state class that can be handy for dealing with the
 * `target_i' fields found in most of the stub presentations in PRES_C.  When
 * it digs the object reference out of the C function parameters (or whatever),
 * it simply invokes a macro/function "flick_<be>_<op>_target".
 */
struct target_mu_state : public mu_state
{
	target_mu_state(be_state *_state, mu_state_op _op, int _assumptions,
			const char *which);
	
	/*
	 * We should never need these when marshaling an object target, so
