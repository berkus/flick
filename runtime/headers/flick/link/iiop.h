...
#define flick_iiop_client_start_encode() {				     \
	...
	flick_iiop_build_message_header(_buf_start, 0);			     \
...
#define flick_iiop_client_start_decode() {				\
	...
	flick_iiop_verify_message_header(_buf_start, 1);		\
...
#define flick_iiop_server_start_encode() {				\
	_stream = OutHeadP;						\
	flick_iiop_build_message_header(_buf_start, 1); /* Reply = 1 */	\
...
#define flick_iiop_client_encode_target(ref, _ofs, ENCNAME, _onerror) {	\
	...
	flick_iiop_encode_new_glob(7 + __len, _onerror);		\
