...
itAlloc(void)
{
	...
	0,			/* u_int itSize */
	1,			/* u_int itNumber */
	TRUE,			/* mig_boolean_t itInLine */
	FALSE,			/* mig_boolean_t itLongForm */
	d_NO,			/* dealloc_t itDeallocate */
	strNULL,		/* string_t itInNameStr */
	strNULL,		/* string_t itOutNameStr */
	flNone,			/* ipc_flags_t itFlags */
	TRUE,			/* mig_boolean_t itPassByValue */
	FALSE,			/* mig_boolean_t itString */
	FALSE,			/* mig_boolean_t itVarArray */
	FALSE,			/* mig_boolean_t itIndefinite */
...
mig_boolean_t
itCheckIsLong(const ipc_type_t *it, ipc_flags_t flags, mig_boolean_t dfault,
	      identifier_t name)
{
    mig_boolean_t islong = dfault;
...
itPrintDecl(identifier_t name, const ipc_type_t *it)
{
	...
	if (it->itNumber == 0 || it->itIndefinite)
	    ...
	    fprintf(stderr,"array [*:%d] of ", it->itNumber);
    }
    else if (it->itPassByValue && ((it->itNumber != 1) ||
			      (it->itInName == MACH_MSG_TYPE_STRING_C)))
	fprintf(stderr,"struct [%d] of ", it->itNumber);
    else if (it->itNumber != 1)
	fprintf(stderr,"array [%d] of ", it->itNumber);

    if (streql(it->itInNameStr, it->itOutNameStr))
	fprintf(stderr,"(%s,", it->itInNameStr);
    else
	fprintf(stderr,"(%s|%s", it->itInNameStr, it->itOutNameStr);
    
    fprintf(stderr," %d%s%s)\n", it->itSize,	it->itLongForm ? ", IsLong" : "",	   it->itDeallocate == d_YES ? ", Dealloc"
...
itPtrDecl(ipc_type_t *it)
{
	...
	error("IPC type decl is too complicated");
    /* This is a bug in MIG: OOL fixed-size data has itNumber set to
       0, thus NOTHING will be sent!  This must retain the array size,
       and checks must use the !itInLine to check for OOL data, not
       itNumber == 0. */
    /* it->itNumber = 0; */
    it->itIndefinite = FALSE;
    it->itInLine = FALSE;
    it->itPassByValue = TRUE;
    it->itString = FALSE;
 ...
 */
ipc_type_t *
itCStringDecl(u_int count, mig_boolean_t varying)
{
    register ipc_type_t *it;
