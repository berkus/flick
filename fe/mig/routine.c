...
argAlloc(void)
{
	...
	FALSE,			/* mig_boolean_t argLongForm */
	FALSE,			/* mig_boolean_t argServerCopy */
	FALSE,			/* mig_boolean_t argCountInOut */
	...
	0,			/* int argRequestPos */
	0,			/* int argReplyPos */
	FALSE,			/* mig_boolean_t argByReferenceUser */
	FALSE			/* mig_boolean_t argByReferenceServer */
    };
...
rtPrintArg(register const argument_t *arg)
{
    ...
    {
      case akeRequestPort:
	fprintf(stderr,"RequestPort");
	break;
      case akeReplyPort:
	fprintf(stderr,"ReplyPort");
	break;
      case akeWaitTime:
	fprintf(stderr,"WaitTime");
	break;
      case akeMsgOption:
	fprintf(stderr,"MsgOption");
	break;
      case akeMsgSeqno:
	fprintf(stderr,"MsgSeqno\t");
	break;
      default:
    ...
    fprintf(stderr,"\t%s: %s", arg->argName, it->itName);
	...
	if (arg->argDeallocate == d_YES)
	    ...
	    fprintf(stderr,", NotDealloc");
    }

    if (arg->argLongForm != it->itLongForm) {
	if (arg->argLongForm)
	    fprintf(stderr,", IsLong");
	else
	    fprintf(stderr,", IsNotLong");
    }

    if (arg->argServerCopy)
	fprintf(stderr,", ServerCopy");

    if (arg->argCountInOut)
	fprintf(stderr,", CountInOut");
}
...
rtPrintRoutine(register const routine_t *rt)
{
    ...
    fprintf(stderr,"%s (%d) %s(", rtRoutineKindToStr(rt->rtKind),
...
static void
rtCheckSimpleIn(const argument_t *args, u_int mask, mig_boolean_t *fixed,
		mig_boolean_t *simple)
{
    register const argument_t *arg;
    mig_boolean_t MayBeComplex = FALSE;
    mig_boolean_t MustBeComplex = FALSE;
...
static void
rtCheckSimpleOut(const argument_t *args, u_int mask, mig_boolean_t *fixed,
		 mig_boolean_t *simple)
{
    register const argument_t *arg;
    mig_boolean_t MayBeComplex = FALSE;
...
rtFindSize(const argument_t *args, u_int mask)
{
	...
	if (akCheck(arg->argKind, mask))
	{
		...
		register u_int bs = (it->itSize / 8); /* in bytes */
...
rtCheckMaskFunction(const argument_t *args, u_int mask,
		    mig_boolean_t (*func)(const argument_t *))
{
    register const argument_t *arg;
...
rtAddByReference(register routine_t *rt)
{
	...
	if (akCheck(arg->argKind, akbReturnRcv) &&
	    (it->itPassByValue || it->itIndefinite)) {
