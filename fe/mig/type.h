 ...
 *
 * At most one of itPassByValue and itString should be TRUE.  If both are
 * false, then this is assumed to be an array type (msg data is passed
 * by reference).  If itPassByValue is TRUE, then msg data is passed by value
 ...
 * then itPassByValue and itString are FALSE.  If it is out-of-line, then
 * itPassByValue is TRUE (because pointers can be assigned).
 *
...
typedef struct ipc_type
{
    ...
    u_int itSize;
    u_int itNumber;
    mig_boolean_t itInLine;
    mig_boolean_t itLongForm;
    ...
    mig_boolean_t itPassByValue;
    mig_boolean_t itString;
    mig_boolean_t itVarArray;
    mig_boolean_t itIndefinite;
...
extern ipc_type_t *itCStringDecl(u_int number, mig_boolean_t varying);
...
extern mig_boolean_t itCheckIsLong(const ipc_type_t *it, ipc_flags_t flags,
				   mig_boolean_t dfault, identifier_t name);
